...
"""
Qwen-OFT Framework with Block-Level Mixture-of-Experts

An extension of QwenOFT that replaces the vanilla MLPResNet action head with
block-level MoE (MoEMLPResNetBlock).  Each residual block is replaced by k
parallel expert FFNs gated by a softmax router with top-k sparse routing
and load-balancing aux loss.

Key differences from QwenOFT:
  - action_model uses MLPblocklevelmoe.get_action_model()
  - Default MoE config: num_experts=4, top_k=2, balance_loss_weight=0.01
  - forward() returns aux_loss alongside action_loss
  - Registry name: "QwenOFTblocklevelmoe"
...
from starVLA.model.modules.action_model.MLPblocklevelmoe import get_action_model
from starVLA.model.modules.vlm import get_vlm_model
from starVLA.training.trainer_utils.trainer_tools import resize_images


# ──────────────────────────────────────────────────────────────────────
#  Default Config for QwenOFTblocklevelmoe
#  - Documents every framework-level parameter with type + description
#  - YAML values override these defaults; extra YAML keys are preserved
# ──────────────────────────────────────────────────────────────────────
@dataclass
class QwenOFTBlockLevelMoEDefaultConfig:
    """QwenOFT + block-level MoE framework default parameters.
    ...
    # === Action head (block-level MoE regression over action special tokens) ===
    action_model: dict = field(
        default_factory=lambda: {
            ...
            "moe_level": "block",          # MoE granularity (only "block" for now)
            "num_experts": 4,              # number of expert FFNs per block
...
@FRAMEWORK_REGISTRY.register("QwenOFTblocklevelmoe")
class Qwenvl_OFT_MoE(baseframework):
    """
    Multimodal vision-language-action model (OFT variant with block-level MoE).

    Components:
      - Qwen2.5-VL / Qwen3-VL backbone for fused language/vision token embeddings
      - Action special token injected into the VLM sequence
      - Block-level MoE regression head over action token hidden states
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = merge_framework_config(QwenOFTBlockLevelMoEDefaultConfig, config)
