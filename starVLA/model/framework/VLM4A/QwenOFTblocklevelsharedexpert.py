...
"""
Qwen-OFT Framework with Block-Level MoE + Shared Expert

An extension of QwenOFT that replaces the vanilla MLPResNet action head with
block-level MoE + an always-on shared expert (MoEResNetBlockWithSharedExpert).
Each residual block is replaced by k parallel task expert FFNs + 1 shared expert
FFN, gated by a softmax router with top-k sparse routing and load-balancing aux loss.

Key differences from QwenOFT:
  - action_model uses MLPblocklevelsharedexpert.get_action_model()
  ...
  - Registry name: "QwenOFTblocklevelsharedexpert"
"""
...
from starVLA.model.modules.action_model.MLPblocklevelsharedexpert import get_action_model
from starVLA.model.modules.vlm import get_vlm_model
from starVLA.training.trainer_utils.trainer_tools import resize_images


# ──────────────────────────────────────────────────────────────────────
#  Default Config for QwenOFTBlockLevelSharedExpert
#  - Documents every framework-level parameter with type + description
#  - YAML values override these defaults; extra YAML keys are preserved
# ──────────────────────────────────────────────────────────────────────
@dataclass
class QwenOFTBlockLevelSharedExpertDefaultConfig:
    """QwenOFT + block-level MoE + shared expert framework default parameters.
    ...
    name: str = "QwenOFTblocklevelsharedexpert"
    ...
    # === Action head (block-level MoE + shared expert regression) ===
    action_model: dict = field(
        default_factory=lambda: {
            ...
            "use_moe": True,               # enable block-level MoE
            "moe_level": "block",          # MoE granularity (only "block")
            "num_experts": 4,              # number of task expert FFNs per block
            "top_k": 2,                    # top-k sparse routing (0 / None = dense)
            "balance_loss_weight": 0.01,   # weight of load-balancing aux loss
            "use_shared_expert": True,     # add always-on shared expert
            "shared_expert_scale": 0.5,    # scale factor for shared expert output
        }
    )


@FRAMEWORK_REGISTRY.register("QwenOFTblocklevelsharedexpert")
class Qwenvl_OFT_MoE_SharedExpert(baseframework):
    """
    Multimodal vision-language-action model (OFT variant with block-level MoE + shared expert).

    Components:
      - Qwen2.5-VL / Qwen3-VL backbone for fused language/vision token embeddings
      - Action special token injected into the VLM sequence
      - Block-level MoE regression head with always-on shared expert over action token hidden states
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = merge_framework_config(QwenOFTBlockLevelSharedExpertDefaultConfig, config)
        ...
        self.action_token = "\U0001f50d"  # magnifying glass emoji
    ...
    # Discretised state -> instruction prefix (pi0.5 style)
...
if __name__ == "__main__":
    ...
    model = Qwenvl_OFT_MoE_SharedExpert(cfg)
    print(model)
