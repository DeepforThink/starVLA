...
"""
Qwen-OFT Framework with Linear-Level Mixture-of-Experts

An extension of QwenOFT that replaces the vanilla MLPResNet action head with
linear-level MoE (MoELinearResNetBlock).  Only the Linear(dim, dim) inside
each residual block is replaced by k parallel expert Linear layers gated by a
softmax router with top-k sparse routing and load-balancing aux loss.

Key differences from QwenOFT:
  - action_model uses MLPlinearlevelmoe.get_action_model()
  - Default MoE config: num_experts=4, top_k=2, balance_loss_weight=0.01
  - forward() returns aux_loss alongside action_loss
  - Registry name: "QwenOFTlinearlevelmoe"
...
from starVLA.model.modules.action_model.MLPlinearlevelmoe import get_action_model
from starVLA.model.modules.vlm import get_vlm_model
from starVLA.training.trainer_utils.trainer_tools import resize_images


# ──────────────────────────────────────────────────────────────────────
#  Default Config for QwenOFTlinearlevelmoe
#  - Documents every framework-level parameter with type + description
#  - YAML values override these defaults; extra YAML keys are preserved
# ──────────────────────────────────────────────────────────────────────
@dataclass
class QwenOFTLinearLevelMoEDefaultConfig:
    """QwenOFT + linear-level MoE framework default parameters.
    ...
    name: str = "QwenOFTlinearlevelmoe"
    ...
    # === Action head (linear-level MoE regression over action special tokens) ===
    action_model: dict = field(
        default_factory=lambda: {
            ...
            "num_experts": 4,              # number of expert Linear layers per block
...
@FRAMEWORK_REGISTRY.register("QwenOFTlinearlevelmoe")
class Qwenvl_OFT_LinearMoE(baseframework):
    """
    Multimodal vision-language-action model (OFT variant with linear-level MoE).

    Components:
      - Qwen2.5-VL / Qwen3-VL backbone for fused language/vision token embeddings
      - Action special token injected into the VLM sequence
      - Linear-level MoE regression head over action token hidden states
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = merge_framework_config(QwenOFTLinearLevelMoEDefaultConfig, config)
...
if __name__ == "__main__":
    ...
    model = Qwenvl_OFT_LinearMoE(cfg)
    print(model)
